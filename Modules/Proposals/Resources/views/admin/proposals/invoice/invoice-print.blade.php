...
<a href="{{route('admin.proposals.show', $invoice->id)}}" class="btn btn-info" role="button">@lang('custom.invoices.back-to-proposals')</a>
...
<style>
   ...
   .wrapper{
        ...
        <header><br/>
            <h1>{{trans('proposals::custom.proposals.title-single')}}</h1>
            <address>
                <h4 class="red"><strong>{{trans('proposals::custom.proposals.proposal_no') . $invoice->invoicenumberdisplay}}</strong></h4>
                @if(! empty($invoice->reference) )
                <h4 class="red"><strong>{{trans('proposals::custom.proposals.reference') . $invoice->reference}}</strong></h4>
                @endif
                ...
                if ( 'accepted' == $invoice->paymentstatus ) {
                    $class = 'success';
                }
                if ( 'rejected' == $invoice->paymentstatus ) {
                    $class = 'danger';
                }
                
                if ( 'delivered' == $invoice->paymentstatus ) {  
                    $class = 'info';
                }
            ...
            <tr><td class="beta2"><img alt="" src="{{asset( 'uploads/settings/' . $logo )}}" height="56" width="180"></td></tr> 
                ...
                <tr><td class="beta2"><strong>{{getSetting('Company_Name_On_Proposal', 'proposal-settings', trans('global.global_title'))}}</strong></td></tr>
                <tr><td class="beta2">{{getSetting('Company-Address', 'proposal-settings')}}</td></tr>
            </table>
            <address>
                <p><strong>{{trans('proposals::custom.proposals.recipient')}}</strong></p>
                 <?php
        ...
        <article>
            ...
            <table class="balance">
                <tr>
                    ...
                    <td><span class="amount">{{digiCurrency($total_tax, $invoice->currency_id)}}</span></td>
                </tr>
                <tr>
                    <th><span>@lang('custom.products.sub_total')</span></th>
                    <td><span class="amount">{{digiCurrency($sub_total, $invoice->currency_id)}}</span></td>
                </tr>
                <tr>
                    <th><span>@lang('custom.products.total_discount')</span></th>
                    <td><span class="amount">{{digiCurrency($total_discount, $invoice->currency_id)}}</span></td>
                </tr>
                <tr>
             ...
             <article>
                <table class="beta4 pagebreak">
                 @if( $invoice->recurring_period_id )
                     <tr><td class="beta4"><b>@lang('proposals::custom.proposals.payment-terms') : </b></td></tr>
    ...
    {{ getSetting('proposal-footer', 'proposal-settings') }}</p>
    </div>  
    @endif
