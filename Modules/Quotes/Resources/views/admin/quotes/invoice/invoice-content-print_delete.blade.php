...
<header>
			<h1 style="background: linear-gradient(to right, rgb(0, 201, 255), rgb(146, 254, 157));">{{ strtoupper( trans('quotes::custom.quotes.title-single') ) }}</h1>
			...
			<?php
            $logo = getSetting('Invoice-Logo', 'quote-settings');
            if ( empty( $logo ) ) {
...
<p style="font-size: 13px; float: right; font-weight: 700; margin-top: -160px; margin-right: -20px;">{{getSetting('site_address', 'site_settings')}}</p></span>
		...
		<article>
			...
			<table class="balance" style="/*! margin-right: 90px; */;">
				<tr>
					<th><span>{{trans('quotes::custom.quotes.quote_no')}}</span></th>
                ...
                <?php
                $show_sale_agent_on_invoices = getSetting('show_sale_agent_on_invoices', 'quote-settings');
			...
			<table class="balance" style="/*! margin-right: 90px; */ margin-top: -20px;">
				<tr>
 ...
 <div class="column" >
    <header >
			<address solox style="width: 300px;">
				...
				<p>@lang('quotes::custom.quotes.payment-terms') : <b>{{ $invoice->recurring_period->title }}</b></p>
				@endif
				
				@if(! empty( $invoice->invoice_notes ) )
	                <p>@lang('global.quotes.fields.proposal-text')</p>
  ...
  <div class="column" style="float:right;">
    <header>
			<address>
				...
				<img alt="" src="{{asset( 'uploads/settings/' . $authorized_sign )}}" style="/*! height: 40px; */ width: 100px;/*! float: right; */margin-left: 190px;/*! width: auto; */"></span>
				@endif
				...
				<br><span style="float:right; /*! padding-top: 40px; *//*! margin-right: 0px; */">{{$authorized_designation}}</span>
				@endif
			</address>
		</header>
</div>
