...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$('#invoiceSend').click(function() {
			...
			if ( 'make-payment-pay' == action ) {
				...
				if ( errors == 0 ) {
					$.ajax({
		                url: '{{url('admin/contracts/save-payment')}}',
		                dataType: "json",
		                method: 'post',
		                data: {
		                	action: action,
	...
	function loadEmailTemplate (invoice_id, action) {
		...
		jQuery.ajax({
	        url: baseurl + '/admin/contracts/mail-invoice',
	        type: 'POST',
	        data: {
	        	'_token': crsf_hash,
	...
	$('.convertContract').click(function() {
		...
		if ( invoice_id != '' ) {
			var invurl = '';
			if ( invoice_url != '' ) {
				invurl = '&nbsp;<a href="'+invoice_url+'" target="_blank">{{trans("contracts::custom.contracts.view-now")}}</a>';
			}
			str = '<p><code>{{trans("contracts::custom.contracts.contract-already-convert")}}</code>'+invurl+'</p>';
		}

		bootbox.confirm({
		    title: "{{trans('contracts::custom.contracts.convert-to-invoice')}}",
		    message: str + "{{trans('contracts::custom.contracts.convert-to-invoice-are-you-sure')}}",
		    buttons: {
		        cancel: {
